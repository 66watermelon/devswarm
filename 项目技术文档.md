...
- **速率限制：** `slowapi` + Redis（端口 6380, db 2），`POST /api/chat/stream` 10/min/IP，`/register` 5/min，`/login` 10/min，全局默认 60/min
- **输入消毒：** Pydantic `field_validator` 拒绝 prompt injection 关键字（`ignore previous`、`<<SYS>>`、`<|im_start|>`），移除不可见控制字符，prompt 上限 5000 字符，user_code 上限 20000 字符
- **分布式锁：** Redis `SETNX` 防同一 thread 并发推演，600s TTL + Worker `finally` 无条件释放
- **JWT 吊销：** 登出时立即删除 Redis jti，令牌即时失效
